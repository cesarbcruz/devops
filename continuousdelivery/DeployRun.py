    ...
    if activate_vpn:
        try:
            ...
            if file_ovpn:
                ...
                kill_vpn()
                p = subprocess.Popen(['openvpn', '--config', file_ovpn, '--auth-user-pass', name_file_auth], stdout=subprocess.PIPE, stderr=subprocess.PIPE, cwd=folder_vpn_certificate_user, close_fds=True, shell=True)
